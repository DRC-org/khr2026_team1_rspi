...
# gz 共分散: 移動中は固定値、静止中は高くして残留バイアスの EKF 積分を防ぐ
_GZ_COV = 0.05
...
class ImuPublisherNode(Node):
    ...
    def _on_feedback(self, msg: WheelMessage) -> None:
        ...
        # 静止中は残留バイアスの EKF 積分を防ぐため共分散を高くする
        imu_msg.angular_velocity_covariance[8] = _GZ_COV_STATIONARY if is_stationary else _GZ_COV
